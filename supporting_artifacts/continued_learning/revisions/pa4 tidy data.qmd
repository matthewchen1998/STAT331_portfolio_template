...
```{r}
 military_clean<-anti_join(military_clean,cont_region,by = 
                             c("Country" = "Region"))
...
::: callout-tip
Looking at the dataset, you should notice that the continents included in the `Country` column have missing values for the `1988` to `2019` columns. Use this information to locate what continents still remain!
:::

::: callout-caution
Using `across()` in `filter()` was deprecated in dplyr 1.0.8. It is advised to use `if_any()` or `if_all()` instead. For help on these functions, pull up their help file!

```{r}

military_clean|>
  filter(if_all(`1988`:`2019`,is.na))
...
::: callout-caution
**Do not** overwrite your cleaned up dataset! Save your longer data as a new object!
:::
...
```{r}
ggplot(data = military_clean_longer, mapping = aes(x = Spending, y = Year)) +
...
#military_clean_longer|>
  #filter(Year == "1990")
 # summarize(Spending = n())


#ggplot(data = military_clean_longer, mapping = aes(x = Spending, y = Country)) +
  #geom_boxplot(outlier.shape = NA)
